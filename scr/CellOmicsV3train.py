...
parser.add_argument('--mut', type=float, default=3.,
                    help='coefficient of mutual loss (default: 3.), not used for training')
...
def saving_model(model, model_num, epoch, seed):
    if model_num == 1:
        model_dir = dir_name+"/save_model1s/"+str(seed)+"/"
    elif model_num == 2:
        model_dir = dir_name+"/save_model2s/"+str(seed)+"/"
    else:
...
def train_omics_one_epoch(epoch, autoencoder, dataloader, writer, downstream_task='3typesclassification'):
    ...
    print(f'{epoch+1} DONE\n Training Loss: {total_loss} (Simi:{total_loss_feature}, TCR:{total_loss_TCR}, MSE:{total_loss_restoring}, Mutual:{total_loss_mutual})')
    return total_loss
...
if __name__ == '__main__':
    train(train_dl, val_dl, test_dl, writer, seed)
