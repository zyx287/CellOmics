...
Date: 2023-09-21
Description:
    Formated training script for CellOmicsV2
Update: 2023-09-27
    Add downstream task
'''
############
...
parser.add_argument('--dir', type=str, default="CellOmicsV2",
                    help='directory name (default: CellOmicsV2)') 
...
parser.add_argument('--downstreamtask', type=str, default="None",
...
dir_name = f"/shared/projects/autoencoder/analysis/AnalysisResult/CellOmics/{args.dir}/{args.downstreamtask}/{date_info}_{args.data}_numpatch{args.num_patches}_bs{args.bs}_{args.msg}"
...
if args.data == "emt-cell" or args.data == "ips-cell":
    all_dataset = load_dataset(data_name=args.data, num_patch = num_patches, transform_type=args.transform_type, normalize=args.normalize, downstream_task=args.downstreamtask)
...
if args.downstreamtask == 'None':
...
elif args.downstreamtask in ['3typesclassification','2typesclassification','3typesPCAregression','2typesPCAregression','3typesLevelregression','2typesLevelregression']:
    # Model1
    model1 = AutoEncoder(encoder(z_dim=512, hidden_dim=512, arch = args.arch), Decoder(latent_dim=512))
    model1 = nn.DataParallel(model1)
    model1.to(device)
    ...
    if args.downstreamtask == '3typesclassification':
        model2 = MLPRegressionModel(input_dim=512, hidden_dim=256, output_dim=3)
        criterion = nn.CrossEntropyLoss()
    elif args.downstreamtask == '2typesclassification':
        model2 = MLPRegressionModel(input_dim=512, hidden_dim=256, output_dim=2)
        criterion = nn.CrossEntropyLoss()
    elif args.downstreamtask == '3typesPCAregression' or args.downstreamtask == '2typesPCAregression':
        model2 = MLPRegressionModel(input_dim=512, hidden_dim=256, output_dim=2)
        criterion = nn.L1Loss()
    elif args.downstreamtask == '3typesLevelregression' or args.downstreamtask == '2typesLevelregression':
...
def train_omics_one_epoch(epoch, autoencoder, dataloader, writer, downstream_task='3typesclassification'):
    ...
    for datapair in dataloader:
        ...
        loss_feature, _ = feature_loss(list_proj, avg_proj)
        ...
        loss = args.patch_sim*loss_feature - args.tcr*loss_TCR + args.mse*loss_MSE# Combined loss (Maximal the TCR and similarity)
    ...
    print(f'{epoch+1} DONE\n Training Loss: {total_loss} (Simi:{total_loss_feature}, TCR:{total_loss_TCR}, MSE:{total_loss_restoring})')
    return total_loss
...
def train(train_dl, val_dl, test_dl, writer, seed):
    train_best = 99999999
    val_best = 99999999
    if (args.data == 'ips-cell' and (args.downstream_task == '3typesclassification' or args.downstream_task == '2typesclassification') and (not ips_down=args.ips_down)) or args.downstream_task == 'None':
        for epoch in range(args.epoch):
            train_loss =  train_omics_one_epoch(epoch, model1, train_dl, writer)
            val_loss = val_omics_one_epoch(epoch, model1, val_dl, writer)
            test_loss = test_omics_one_epoch(epoch, model1, test_dl, writer)
...
if __name__ == '__main__':
    train(test_dl, val_dl, train_dl, writer, num_patches, dir_name)
